...
class DCGAN:
    def __init__(self, discriminator_path, generator_path, output_directory, img_size, dropout, bn_momentum, adam_lr, adam_beta):
        ...
        labels = f"/dropout={self.dropout} bn_momentum={self.bn_momentum} adam_lr={self.adam_lr} adam_beta={self.adam_beta}"
        self.output_directory = output_directory + labels
    ...
    def build_gan(self):
        optimizer = Adam(self.adam_lr, self.adam_beta)
    ...
    def save_imgs(self, epoch):
        " Generates n images from the model, and saves them"
        n = 1019
        imgs = self.gene_imgs(n)
        imgs = 0.5 * imgs + 0.5

        for i, img_array in enumerate(imgs):
            path = f"{self.output_directory}/generated"
    ...
    def generate_imgs(self, count, threshold, modifier):
        """
        Generates (count) images from the model ensuring the discriminator scores them between the threshold values and saves them
        """
        self.build_gan()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_directory',
        ...
        default=f"data/output/")
    parser.add_argument('--dropout',
        help="Dropout rate for Discriminator",
        default=0.25)
    parser.add_argument('--bn_momentum',
        help="Batch normalization momentum for generator",
        default=0.80)
    parser.add_argument('--adam_lr',
        help="Learning rate for optimizer (both models)",
        default=0.0002)
    parser.add_argument('--adam_beta',
        help="Beta parameter for optimizer (both models)",
        default=0.5)
