...
x_path = "C:/Users/Lantea/Desktop/128_128 side dataset/"
y_path = "C:/Users/Lantea/Desktop/Safe/"
...
def load_imgs(image_path):
    ...
    for file in os.listdir(image_path):
        ...
        data.append(img)
    images = np.expand_dims(np.asarray(data), axis=1)
    size = (images.shape[0], 3, images.shape[2], images.shape[3])
    rgb = np.zeros(size)
    rgb[0::,0::,0::,0::] = images
    rgb[0::,1::,0::,0::] = images
    rgb[0::,2::,0::,0::] = images
...
def get_fid(images1, images2):
    ...
    return fid
with open('fid_brain.csv', 'w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(["Parameters", "FID"])
    x = load_imgs(x_path)
    for dirs in os.listdir(y_path):
        y = load_imgs(os.path.join(y_path, dirs, "generated"))
        writer.writerow([dirs, get_fid(x, y)])
